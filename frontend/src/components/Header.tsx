...
interface HeaderProps {
  ...
  user?: any; // Farcaster user object
}
...
export default function Header({ onOpenScheduler, user }: HeaderProps) {
  ...
  // Profil fotoğrafı önceliği:
  // 1. SDK'dan gelen gerçek URL (user.pfpUrl)
  // 2. Warpcast avatar tahmini (handle ile)
  // 3. Varsayılan boş
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto flex h-14 max-w-4xl items-center justify-between px-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* New Cast Butonu (Sadece Dashboard'da görünür) */}
          {onOpenScheduler && (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="rounded-full h-9 w-9 border border-input ring-offset-background transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              {/* Mobil için 'New Cast' buraya da eklenebilir istersen */}
              {onOpenScheduler && (
